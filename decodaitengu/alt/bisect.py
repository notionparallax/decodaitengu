...
class BisectFindFirstStop:
    ...
    def __call__(self, start, abs_p, gas):
        ...
        if __debug__:
            logger.debug(
                f"find first stop: {start.abs_p}bar -> {abs_p}bar, {start.time}min, n={n}, dt={dt}min"
            )
        assert t >= 0
        ...
        f = lambda k, data: engine._can_ascend(start.abs_p, k * ts_3m + dt, start.data)
        ...
        if k == 0:
            ...
            if __debug__:
                ...
                assert depth % 3 == 0, f"Invalid first stop depth pressure {p}bar ({depth}m)"

                logger.debug(f"find first stop: found at {p}, ascent time={time}")
